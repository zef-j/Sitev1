...
function initDataRoot() {
    ...
    if (!fs.existsSync(activePath)) {
        ...
        if (!fs.existsSync(activePath))
            writeJSON(activePath, { version: 'dev', sections: [] });
    }
}
initDataRoot();
...
function foundationFolderFor(meta) {
    return (meta?.foundationId ||
        meta?.foundationSlug ||
        (meta?.foundationName ? slugify(meta.foundationName) : "") ||
        "f_default");
}
function locateExistingFoundationFolder(buildingId) {
    try {
        const foundationsRoot = path.join(DATA_ROOT, 'orgs', CLIENT_ID, 'foundations');
        if (!fs.existsSync(foundationsRoot))
            return null;
...
function getCurrentJsonPath(meta) {
    return path.join(getBuildingDir(meta), 'current.json');
}
function getFilesDir(meta) {
    return path.join(getBuildingDir(meta), 'files');
}
function getVersionsDir(meta) {
    return path.join(getBuildingDir(meta), 'versions');
}
function getLogsDir(meta) { return path.join(getBuildingDir(meta), 'logs'); }
function getEventsLog(meta) { return path.join(getLogsDir(meta), 'events.log'); }
function logEvent(meta, evt, dataVersion, extra = {}) {
...
function getActiveTemplate() {
    ...
    function findUpwards(startDir, fileNames) {
        try {
            let dir = startDir;
            for (let i = 0; i < 6; i++) {
    ...
    const rootFile = findUpwards(process.cwd(), ['active.json', 'template.example.json', 'template_example.json'])
...
app.get('/buildings/:id/versions', (req, res) => {
    ...
    const list = fs.readdirSync(vdir, { withFileTypes: true })
        ...
        return { versionId: d.name, createdAt: m.createdAt || d.name, dataVersion: m.dataVersion || 0 };
    })
