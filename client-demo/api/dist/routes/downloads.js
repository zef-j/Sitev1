...
function crc32Finalize(crc) { return (~crc) >>> 0; }
async function writeToRes(res, buf) {
    if (res.destroyed)
...
function createZipCtx(res) {
    return { res, offset: 0, central: [], entries: 0 };
}
async function addZipEntryFromBuffer(ctx, name, data, mtime) {
    const res = ctx.res;
    ...
    await writeToRes(res, lh);
    ctx.offset += lh.length;
    await writeToRes(res, nameBuf);
    ctx.offset += nameBuf.length;
    let crc = 0xFFFFFFFF;
    crc = crc32Update(crc, data);
    const size = data.length >>> 0;
    await writeToRes(res, data);
    ...
    await writeToRes(res, dd);
    ctx.offset += dd.length;
    // Central directory
...
async function addZipEntryFromPath(ctx, name, filePath, mtime) {
    const res = ctx.res;
    const fileName = zipName(name);
    const nameBuf = Buffer.from(fileName, 'utf8');
    const { dosTime, dosDate } = dosDateTime(mtime);
...
async function addZipDir(ctx, name, mtime) {
...
async function finalizeZip(ctx) {
    const res = ctx.res;
...
async function addBuildingPayloadToZipStream(ctx, meta, prefix, tplJson) {
    ensureCurrent(meta);
...
router.get('/download/global-overview', async (_req, res) => {
    try {
        ...
        const items = getRegistry().map(meta => {
            ...
            ensureCurrent(m);
            const cur = readJSON(currentJson, {});
