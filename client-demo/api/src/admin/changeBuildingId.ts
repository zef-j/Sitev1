...
type Payload = {
  id?: string; oldId?: string; fromId?: string;
  newId?: string; toId?: string; nextId?: string;
};

function pickId(body: any): { oldId: string, newId: string } {
  const b = body as Payload;
  const oldId = b.id || b.oldId || b.fromId;
  const newId = b.newId || b.toId || b.nextId;
  if (!oldId || !newId) throw Object.assign(new Error('id and newId required'), { status: 400 });
  return { oldId, newId };
}

function normalizeAliases(a?: string[]): string[] | undefined {
  if (!a || !Array.isArray(a)) return undefined;
  const uniq = Array.from(new Set(a.filter(s => !!s && typeof s === 'string')));
  return uniq.length ? uniq : undefined;
}

async function updateCurrentJsonBuildingId(dir: string, newId: string) {
...
router.post('/change-building-id', express.json(), async (req, res) => {
  ...
  try {
    const result = withFileLock(LOCK_DIR, 'registry', () => {
      ...
      const nextEntry: BuildingEntry = { ...entry };
      ...
      try { updateCurrentJsonBuildingId(dstDir, newId); } catch {}
    ...
    if (!result) return; // response already set
