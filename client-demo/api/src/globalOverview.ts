...
function s(x: any): string { return x == null ? '' : String(x); }
...
function monthOrderKey(k: string): number {
...
function summarizeValue(ftype: string, value: any): string {
  ...
  if ((ftype === 'yearTable' || ftype === 'monthTable') && value && typeof value === 'object' && !Array.isArray(value)) {
    ...
    keys.sort((a,b) => {
      if (ftype === 'monthTable') return monthOrderKey(a) - monthOrderKey(b);
  ...
  if (ftype === 'file') {
    if (value && typeof value === 'object' && (value.fileId || value.url || value.name || value.storedName)) return '1 fichier';
    return '—';
  }
...
export async function buildGlobalOverviewBuffer(templateJson: any, items: Array<{label: string, current: AnyObj}>): Promise<Buffer> {
  ...
  // Flatten template into rows
  type RowMeta = { secTitle: string; subTitle: string; label: string; path: string; type: string };
  const rows: RowMeta[] = [];
  ...
  // widths & view
  for (let i=1; i<=5; i++) ws.getColumn(i).width = staticCols[i-1].width!;
  for (let i=6; i<6+dynCols.length; i++) ws.getColumn(i).width = 28;
