...
export function resolveDataRoot(): string {
  ...
  try {
    ...
    if (cfg?.DATA_ROOT && fs.existsSync(cfg.DATA_ROOT)) return cfg.DATA_ROOT;
    if (cfg?.default?.DATA_ROOT && fs.existsSync(cfg.default.DATA_ROOT)) return cfg.default.DATA_ROOT;
  } catch {}
  try {
