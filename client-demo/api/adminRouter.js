...
export default function createAdminRouter() {
  ...
  router.post('/change-building-id', async (req,res) => {
    ...
    try {
      ...
      // Move folders and update current.json
      for (const d of dirs) {
  ...
  router.post('/change-foundation-id', async (req,res) => {
    ...
    const nid = slugify(newId);
    const reg = await readJson(REGISTRY_FILE);
    const exists = reg.some(x => x.foundationId === nid);
    if (exists) return res.status(409).json({ error: 'newId already exists' });

    const fdirs = await findFoundationDirs(oldId);
